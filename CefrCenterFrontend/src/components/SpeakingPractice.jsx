...
import { Mic, Search, Filter, Loader, AlertCircle } from "lucide-react";
...
const SpeakingComponent = () => {
  ...
  const TYPES = ["all", "speaking-topic", "daily-conversation", "ielts-style", "self-introduction", "discussion-prompt"];
  ...
  const fetchExercises = async (page = 1, search = "", level = "all", type = "all") => {
    ...
    setError(null);
    try {
      let query = `${BACKEND_URL}/api/speaking?page=${page}&limit=${ITEMS_PER_PAGE}`;
  ...
  const TypeBadge = ({ type }) => {
    const colors = {
      "speaking-topic": "bg-blue-100 text-blue-800",
      "daily-conversation": "bg-green-100 text-green-800",
      "ielts-style": "bg-purple-100 text-purple-800",
      "self-introduction": "bg-yellow-100 text-yellow-800",
      "discussion-prompt": "bg-pink-100 text-pink-800",
    };
    return (
  ...
  return (
    <div className={`min-h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      {/* Header */}
      <div className={`${darkMode ? "bg-gray-800" : "bg-gradient-to-r from-green-600 to-green-700"} text-white py-12 px-4`}>
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-3 mb-4">
            <Mic size={32} />
            <h1 className="text-4xl font-bold">Speaking Practice</h1>
          </div>
          <p className={`${darkMode ? "text-green-300" : "text-green-100"}`}>Improve your pronunciation and fluency</p>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {TYPES.slice(0, 3).map((type) => (
                  ...
                  >
                    {type === "all" ? "All" : type.split("-")[0].toUpperCase()}
                  </button>
                ))}
                <select
                  value={selectedType}
          ...
          <div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {exercises.map((exercise) => (
                ...
                >
                  <div className={`${darkMode ? "bg-green-900" : "bg-green-50"} p-4 flex items-center gap-3`}>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {exercise.keyVocabulary?.slice(0, 3).map((word, idx) => (
                        <span
                          key={idx}
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className={`text-lg ${darkMode ? "text-gray-300" : "text-gray-700"}`}>{selectedExercise.prompt}</p>
                </div>

                <div>
                  <h3 className="font-bold mb-2">Description</h3>
                  <p className={darkMode ? "text-gray-300" : "text-gray-700"}>{selectedExercise.description}</p>
                </div>

                {selectedExercise.context && (
                  <div>
                    <h3 className="font-bold mb-2">Context</h3>
                    <p className={darkMode ? "text-gray-300" : "text-gray-700"}>{selectedExercise.context}</p>
                  </div>
                )}

                {selectedExercise.followUpQuestions && selectedExercise.followUpQuestions.length > 0 && (
                  <div>
                    <h3 className="font-bold mb-2">Follow-up Questions</h3>
                    <ul className={`space-y-1 ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                      {selectedExercise.followUpQuestions.map((q, idx) => (
                        <li key={idx}>• {q}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {selectedExercise.sampleResponse && (
                  <div>
                    <h3 className="font-bold mb-2">Sample Response</h3>
